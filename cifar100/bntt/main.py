...
from model import *
from dataprocess import CIFAR100C
...
parser.add_argument('--leak_mem',              default=0.99,
                    type=float, help='Leak_mem')
parser.add_argument('--num_epochs',            default=100,
                    type=int,   help='Number of epochs')
parser.add_argument('--num_workers',           default=4,
                    type=int, help='number of workers')
parser.add_argument('--domain',                default='cifar100',
...
user_foldername = 'cifar100vgg11_timestep30_lr0.3_epoch100_leak0.99'
...
if args.domain == 'cifar100':
    test_set = torchvision.datasets.CIFAR100(root='/path/to/cifar100', train=False,
                                             download=True, transform=transform_test)
else:
    adapt_set = CIFAR100C(root='/path/to/cifar100/CIFAR-100-C', name=args.domain,
                          transform=None, level=5)
    test_set = CIFAR100C(root='/path/to/cifar100/CIFAR-100-C', name=args.domain,
...
if args.phase == 'test':
    print('********** SNN testing **********')
    model = SNN_VGG11_BNTT(
        num_steps=num_steps, leak_mem=leak_mem, img_size=img_size, num_cls=num_cls).cuda()
    ...
    for j in range(len(adapt_set)):
        ...
        model = SNN_VGG11_BNTT(num_steps=num_steps, leak_mem=leak_mem,
        ...
        # Adaptation
        model = adapt(model, optimizer_adapt, image, niter=1,
                      batch_size=32, method=args.method)
