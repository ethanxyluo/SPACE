...
def _augmix_aug(x_orig, l):
    x_orig = preaugment(x_orig)
    x_processed = preprocess(x_orig)
    w = np.float32(np.random.dirichlet([1, 1]))
    ...
    for i in range(2):
        ...
        for _ in range(np.random.randint(1, 4)):
            x_aug = np.random.choice(augmentations)(x_aug, l)
...
def _single_aug(x_orig):
    ...
    x_aug = x_orig.copy()
    for _ in range(np.random.randint(1, 4)):
        x_aug = np.random.choice(augmentations)(x_aug, 1)
    return x_aug


aug32_single = _single_aug
...
def rand_lvl(n):
    return np.random.uniform(low=0.1, high=n)  # origina_low=0.1
    # return n
...
augmentations = [
    autocontrast,
    equalize,
    lambda x, l: rotate(x, l),
    lambda x, l: solarize(x, l),
    lambda x, l: shear_x(x, l),
    lambda x, l: shear_y(x, l),
    lambda x, l: translate_x(x, l),
    lambda x, l: translate_y(x, l),
    lambda x, l: posterize(x, l),
]
